...
void GLContext::gl_clip_plane(GLenum plane, GLdouble const* equation)
{
    ...
    m_clip_plane_attrib.eye_clip_plane[plane_idx] = m_model_view_matrix * eqn;
...
void GLContext::gl_get_clip_plane(GLenum plane, GLdouble* equation)
{
    ...
    auto plane_idx = static_cast<size_t>(plane) - GL_CLIP_PLANE0;
    equation[0] = static_cast<GLdouble>(m_clip_plane_attrib.eye_clip_plane[plane_idx][0]);
    equation[1] = static_cast<GLdouble>(m_clip_plane_attrib.eye_clip_plane[plane_idx][1]);
    equation[2] = static_cast<GLdouble>(m_clip_plane_attrib.eye_clip_plane[plane_idx][2]);
    equation[3] = static_cast<GLdouble>(m_clip_plane_attrib.eye_clip_plane[plane_idx][3]);
}
...
void GLContext::sync_clip_planes()
{
    ...
    for (size_t plane_idx = 0; plane_idx < 6; ++plane_idx) {
        if ((m_clip_plane_attrib.enabled & (1 << plane_idx)) != 0u) {
            user_clip_planes.append(m_clip_plane_attrib.eye_clip_plane[plane_idx]);
        }
    }
